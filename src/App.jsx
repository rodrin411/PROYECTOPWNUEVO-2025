...
function Home() {
  ...
  // 2. Estado para los streams reales
  const [streamsEnVivo, setStreamsEnVivo] = useState([]);

  // Streams falsos (de relleno)
  const streamsFalsos = [
  ...
  useEffect(() => {
    ...
    socket.on('lista_streams', (lista) => {
        setStreamsEnVivo(lista);
    });
    ...
    <div className="principal-container">
      ...
      <section className="streams-section">
        ...
        <div className="streams-grid">
          ...
          {/* 4. RENDERIZAR LOS STREAMS REALES PRIMERO */}
          {streamsEnVivo.map((stream) => (
            <div 
              key={stream.id} 
              className="stream-card"
              onClick={() => irAlStream(stream)} 
              style={{cursor: 'pointer', border: "2px solid #00ffcc"}} // Borde verde para destacar
            >
